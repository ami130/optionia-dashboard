...
import { useGetSingleProfileSettingQuery, useUpdateProfileSettingMutation } from "../api/profileSettingEndPoints";
...
const UpdateProfileSetting = () => {
  ...
  const { data: singleProfileSetting } = useGetSingleProfileSettingQuery(Number(studentId));
  const [update, { isLoading, isSuccess }] = useUpdateProfileSettingMutation();
  ...
  useEffect(() => {
    if (singleProfileSetting) {
      const data = { ...singleProfileSetting.data };
  ...
  }, [form, isSuccess, navigate, phoneFields]);
