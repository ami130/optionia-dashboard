...
import { useGetSingleEarningReportQuery, useUpdateEarningReportMutation } from "../api/earningReportEndPoints";


const { Title, Text } = Typography;

const UpdateEarningReport = () => {
  const { studentId } = useParams();
  const navigate = useNavigate();

  const { data: singleEarningReport } = useGetSingleEarningReportQuery(Number(studentId));
  const [update, { isLoading, isSuccess }] = useUpdateEarningReportMutation();
  ...
  useEffect(() => {
    if (singleEarningReport) {
      const data = { ...singleEarningReport.data };
  ...
  }, [form, isSuccess, navigate, singleEarningReport]);
    ...
    <div className="update-student-form">
      <div className="form-header">
        <Title level={3} className="form-title">
          Update EarningReport Information
        </Title>
      ...
      <Form form={form} onFinish={onFinish} isLoading={isLoading}>
        <Row gutter={[24, 24]}>
          {/* EarningReport Information Section */}
          <Col xs={24}>
            <Card
              title={<span className="section-title">EarningReport Information</span>}
            ...
            >
              <Row gutter={[16, 16]}>
                ...
                <Col xs={24} sm={12}>
                  <Form.Item<any>
                    label={
                      <span className="form-label">Relation to EarningReport</span>
                    }
...
export default UpdateEarningReport;
